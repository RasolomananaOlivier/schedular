    ...
    <table class="w-full text-left text-sm text-gray-500 rtl:text-right">
        ...
        <tr class="border-b bg-white hover:bg-gray-50">
            ...
            <td class="flex justify-end px-6 py-4 text-right">
                <button  onclick="
                        setPopupDescription('Etes-vous sûr de vouloir supprimer la salle \'<%= room.getName() %>\' de la liste ?');
                        const deleteHandler = () => {
                        document.location = '<%= request.getContextPath() + "/rooms/delete?roomId=" + room.getId() %>';
                        };
